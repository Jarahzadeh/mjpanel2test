...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/test-proxy" && request.method === "POST") {
			const { proxy, skip_country, username } = await readJsonBody(request);
...
<div id="support-modal" class="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-black/60  opacity-0 pointer-events-none transition-all duration-300 ease-out">
	<div class="w-full max-w-md bg-white dark:bg-amoled-card border border-red-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
		...
		<div class="space-y-3">
			...
			<a href="https://wa.me/971557256658?text=%D8%B3%D9%84%D8%A7%D9%85%20MJ%2C%20%D9%85%D9%85%D9%86%D9%88%D9%86%20%D8%A7%D8%B2%20%D9%BE%D8%B4%D8%AA%DB%8C%D8%A8%D8%A7%D9%86%DB%8C%20%D9%88%20%D8%B2%D8%AD%D9%85%D8%A7%D8%AA%D8%AA.%20%D9%81%D9%82%D8%B7%20%D8%AE%D9%88%D8%A7%D8%B3%D8%AA%D9%85%20%D8%AA%D8%B4%DA%A9%D8%B1%20%DA%A9%D9%86%D9%85." target="_blank" rel="noopener noreferrer" class="mt-4 w-full inline-flex items-center justify-center gap-2 py-2.5 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-900/20 hover:text-green-800 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-900/40 dark:hover:text-green-400 font-bold rounded-md text-sm transition duration-300">
				پیام تشکر در واتساپ
			</a>
		</div>
	</div>
...
async function testUserSocksProxy() {
	...
	// ابتدا همه لیبل‌ها را روی حالت "در صف" تنظیم می‌کنیم
	...
	// حالا یکی‌یکی (Sequentially) تست می‌کنیم تا از لیمیت مرورگر و کلودفلر جلوگیری شود
	for (let idx = 0; idx < window.proxyFieldsData.length; idx++) {
		...
		const controller = new AbortController();
		const timeoutId = setTimeout(() => controller.abort(), 8000); // افزایش تایم‌اوت به ۸ ثانیه
		try {
			const res = await fetch('/api/test-proxy', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
